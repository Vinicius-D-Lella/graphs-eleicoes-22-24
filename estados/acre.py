...
database = pd.read_csv('data\AC_2022.csv', encoding="latin1", sep=",")
siglas = pd.read_csv('data\SIGLAS-2022.csv', encoding="latin1", sep=",")

title, select = st.columns(2)

with title:
    st.title('AC 2022 - Candidatos a Deputado Federal')
...
federais = []
database = database.groupby(['NR_VOTAVEL',"NM_UE","DS_CARGO","NM_VOTAVEL","ANO_ELEICAO"]).sum({"QT_VOTOS": "sum"})
