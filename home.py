...
selectState, selectYear = st.columns(2)
with selectState:
    state = st.selectbox('Selecione o Estado', [
                            'AC',
                            'AL',
                            'AP',
                            'AM',
...
for candidato_fed in federais.itertuples():
        for r in ranges:
            total = federais[federais['QT_VOTOS'] <= r["range"]["max"]]['QT_VOTOS'].sum()
            porcentagem = total / federais['QT_VOTOS'].sum() * 100
            porcentagems = str(porcentagem).split(".")
            r["Porcentagem de votos até aqui"] = float(porcentagems[0] + "." + porcentagems[1][0:2])
            if r["range"]["min"] <= candidato_fed.QT_VOTOS <= r["range"]["max"]:
                r["candidatos"].append(candidato_fed.NM_VOTAVEL)
                r["Quantidade de Candidatos"] += 1
                r["Votos nesse range"] += candidato_fed.QT_VOTOS
...
st.subheader("Ranking de Partidos escolhidos")
